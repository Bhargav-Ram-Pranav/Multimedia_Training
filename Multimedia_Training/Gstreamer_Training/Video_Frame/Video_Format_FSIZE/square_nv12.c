...
static GstPadProbeReturn frame_probe(GstPad *pad, GstPadProbeInfo *info, gpointer user_data) {
	...
	// Get chroma site information
	 // Get chroma site information
        chroma_site = video_info.chroma_site;

        // Access chroma site information
        g_print("Chroma Site: %d\n", chroma_site);
	
	gsize var=video_info.size;
	g_print("Size of each frame:%ld\n",var);
