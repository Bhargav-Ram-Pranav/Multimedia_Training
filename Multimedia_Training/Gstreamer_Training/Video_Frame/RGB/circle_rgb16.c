...
static GstPadProbeReturn frame_probe(GstPad *pad, GstPadProbeInfo *info, gpointer user_data) {
    ...
    if (gst_video_info_from_caps(&video_info, gst_pad_get_current_caps(pad)) &&
        ...
        guint stride = GST_VIDEO_FRAME_PLANE_STRIDE(&vframe, 0); // Pixel stride (2 bytes per pixel)
        guint pixel_stride=4;
        ...
        for (h = 0; h < height; ++h) {
		    ...
		    guint8 *r_chan=pixel+(0*(8/3));
		    guint8 *g_chan=pixel+(1*(8/3));
		    guint8 *b_chan=pixel+(2*(8/3));
