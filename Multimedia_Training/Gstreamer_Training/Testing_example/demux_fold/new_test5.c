...
int main(int argc, char *argv[]) {
    ...
    // Get the source pad templates for video and audio
    GstPadTemplate *video_src_pad_template = gst_element_class_get_pad_template(GST_ELEMENT_GET_CLASS(demuxer), "video_%u");
    GstPadTemplate *audio_src_pad_template = gst_element_class_get_pad_template(GST_ELEMENT_GET_CLASS(demuxer), "audio_%u");

    // Get the capabilities (caps) of the source pad templates
    video_caps = gst_pad_template_get_caps(video_src_pad_template);
    audio_caps = gst_pad_template_get_caps(audio_src_pad_template);

    g_print("Video source pad capabilities: %s\n", gst_caps_to_string(video_caps));
    g_print("Audio source pad capabilities: %s\n", gst_caps_to_string(audio_caps));
