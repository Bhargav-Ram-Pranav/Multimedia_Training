...
int main(int argc, char *argv[]) {
    ...
    video_src_pad = gst_element_get_static_pad(demuxer, "video_0");
    if(!video_src_pad)
	    g_print("failed to retrieve its data for video src pads\n");
    audio_src_pad = gst_element_get_static_pad(demuxer, "audio_0");
    if(!audio_src_pad)
	    g_print("failed to retrieve its data for audio src pads\n");

    // Get static pads for the queue elements
    GstPad *video_sink_pad = gst_element_get_static_pad(video_queue, "sink");
    if(!video_sink_pad)
	    g_print("failed to retrieve its data for video sink pads\n");
    GstPad *audio_sink_pad = gst_element_get_static_pad(audio_queue, "sink");
    if(!audio_sink_pad)
	    g_print("failed to retrieve its data for audio sink pads\n");
    gst_element_link_pads (demuxer,"video_%u", video_queue, "sink");
