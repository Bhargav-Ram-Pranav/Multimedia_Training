...
int main(int argc, char *argv[]) {
    ...
    // Create a pad for the video queue
    video_sink_pad = gst_element_get_static_pad(video_queue, "sink");
    if (!video_sink_pad) {
        g_printerr("Failed to retrieve the video queue video sink pad.\n");
        gst_object_unref(pipeline);
        return -1;
    }

    // Create a pad for the audio queue
    audio_sink_pad = gst_element_get_static_pad(audio_queue, "sink");
    if (!audio_sink_pad) {
        g_printerr("Failed to retrieve the audio queue audio sink pad.\n");
        gst_object_unref(pipeline);
        return -1;
    }

    // Create a video source pad on the demuxer
    video_src_pad = gst_pad_new_from_static_template(gst_element_class_get_pad_template(GST_ELEMENT_GET_CLASS(demuxer), "video_%u"), "video_0");
    if (!video_src_pad) {
        g_printerr("Failed to create video source pad.\n");
        gst_object_unref(pipeline);
        return -1;
    }

    // Add the video source pad to the demuxer
    if (!gst_pad_set_active(video_src_pad, TRUE) || gst_element_add_pad(demuxer, video_src_pad) != TRUE) {
        g_printerr("Failed to add video source pad to the demuxer.\n");
        gst_object_unref(pipeline);
        return -1;
    }

    // Create an audio source pad on the demuxer
    audio_src_pad = gst_pad_new_from_static_template(gst_element_class_get_pad_template(GST_ELEMENT_GET_CLASS(demuxer), "audio_%u"), "audio_0");
    if (!audio_src_pad) {
        g_printerr("Failed to create audio source pad.\n");
        gst_object_unref(pipeline);
        return -1;
    }

    // Add the audio source pad to the demuxer
    if (!gst_pad_set_active(audio_src_pad, TRUE) || gst_element_add_pad(demuxer, audio_src_pad) != TRUE) {
        g_printerr("Failed to add audio source pad to the demuxer.\n");
        gst_object_unref(pipeline);
        return -1;
    }
