...
int main(int argc, char *argv[]) {
    ...
    // Link the demuxer to the respective queues
    GstPad *video_pad = gst_element_get_static_pad(video_queue, "sink");
    GstPad *audio_pad = gst_element_get_static_pad(audio_queue, "sink");

    if (gst_pad_link(gst_element_get_static_pad(demuxer, "video_0"), video_pad) != GST_PAD_LINK_OK) {
        g_printerr("Video pad link failed.\n");
    }

    if (gst_pad_link(gst_element_get_static_pad(demuxer, "audio_0"), audio_pad) != GST_PAD_LINK_OK) {
