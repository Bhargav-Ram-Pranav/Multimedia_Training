...
GST_DEBUG_OBJECT(&gst_custom_debug, "It is the starting of own plugin");
...
static GstStaticPadTemplate src_factory =GST_STATIC_PAD_TEMPLATE("src",GST_PAD_SRC,GST_PAD_ALWAYS,GST_STATIC_CAPS("ANY"),NULL);
//It creates the sink template for my plugin
static GstStaticPadTemplate sink_factory =GST_STATIC_PAD_TEMPLATE("sink",GST_PAD_SINK,GST_PAD_ALWAYS,GST_STATIC_CAPS("ANY"),NULL);
...
static void gst_custom_init (GstCustom *filter)
{
	...
	gst_pad_set_event_function(filter->sinkpad, gst_event_sink_pad);
	...
	gst_pad_set_chain_function(filter->sinkpad, gst_chain_function);
	...
	//set the src pads
	GST_PAD_SET_PROXY_CAPS(filter->srcpad, TRUE);
	//set the sink pads
	GST_PAD_SET_PROXY_CAPS(filter->sinkpad, TRUE);
