...
int main(int args , char*argv[])
{
	...
	if (!pipeline || !source || !vscale || !filter || !sink || !vconvert || !decodebin) {
	...
	gst_bin_add_many(GST_BIN(pipeline),source , sink,vscale,filter,vconvert,decodebin,NULL);
	...
	gboolean retu = gst_element_link_many(vconvert,filter,vscale,sink,NULL);
	if(retu==FALSE)
	{
