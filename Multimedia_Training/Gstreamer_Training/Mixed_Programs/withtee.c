...
int main(int argc, char *argv[]) {
    ...
    GstElement *pipeline, *filesrc, *demuxer, *video_queue, *video_parse, *video_decode, *video_sink, *audio_queue, *audio_parse, *audio_decode, *audio_sink, *video_tee, *audio_tee;
    ...
    if (!pipeline || !filesrc || !demuxer || !video_queue || !video_parse || !video_decode || !video_sink || !audio_queue || !audio_parse || !audio_decode || !audio_sink || !video_tee || !audio_tee) {
    ...
    gst_bin_add_many(GST_BIN(pipeline), filesrc, demuxer, video_queue, video_parse, video_decode, video_sink, audio_queue, audio_parse, audio_decode, audio_sink, video_tee, audio_tee, NULL);
    ...
    // Link the tee elements to create branches
    if (!gst_element_link(video_tee, video_queue)) {
        g_printerr("Video tee and queue could not be linked.\n");
        gst_object_unref(pipeline);
        return -1;
    }

    if (!gst_element_link(audio_tee, audio_queue)) {
        g_printerr("Audio tee and queue could not be linked.\n");
        gst_object_unref(pipeline);
        return -1;
    }

    // Connect pad-added signals
    g_signal_connect(demuxer, "pad-added", G_CALLBACK(on_pad_added), video_tee);
    g_signal_connect(demuxer, "pad-added", G_CALLBACK(on_pad_added), audio_tee);
...
static void on_pad_added(GstElement *element, GstPad *pad, gpointer data) {
    ...
    GstPad *sinkpad = gst_element_request_pad_simple(tee, "sink_%u");
