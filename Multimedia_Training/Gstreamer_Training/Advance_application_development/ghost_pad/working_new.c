...
int main(int argc, char *argv[]) {
    ...
    if (!gst_element_link_many(audio_queue, audio_parse, audio_decode, audio_sink, NULL)) {
        g_printeaudio_%urr("Audio elements could not be linked.\n");
        gst_object_unref(pipeline);
        return -1;
    }
    //gst_debug_bin_to_dot_file_with_ts(GST_BIN(pipeline), GST_DEBUG_GRAPH_SHOW_ALL, "pipeline");
    ...
    GstMaudio_%uessage *msg = gst_bus_timed_pop_filtered(bus, GST_CLOCK_TIME_NONE, GST_MESSAGE_ERROR | GST_MESSAGE_EOS);
