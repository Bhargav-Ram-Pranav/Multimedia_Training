...
int main(int argc, char *argv[]) {
	...
	gst_bin_add_many(GST_BIN(pipeline), filesrc, demuxer, video_queue, video_parse, video_decode, video_sink, NULL);
	...
	//gst_debug_bin_to_dot_file_with_ts(GST_BIN(mybin), GST_DEBUG_GRAPH_SHOW_ALL, "pipeline");
...
static void on_pad_added(GstElement *element, GstPad *pad, gpointer data) {
	...
	if (g_str_has_prefix(padname, "video")) {
		...
		gst_element_add_pad(mybin,ghostpad);
