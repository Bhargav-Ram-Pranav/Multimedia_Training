...
int main(int argc, char *argv[]) {
    ...
    // Create a new bin
    mybin = gst_bin_new("my-bin");

    // Add all elements to the pipeline and the bin
    gst_bin_add_many(GST_BIN(pipeline), filesrc, demuxer, video_queue, video_parse, video_decode, video_sink,mybin, NULL);
    gst_bin_add_many(GST_BIN(mybin), audio_queue, audio_parse, audio_decode, audio_sink, NULL);

    // Link the elements within the main pipeline
    ...
    // Link the elements within the custom audio bin
    ...
    // Connect pad-added signals for the demuxer
    g_signal_connect(demuxer, "pad-added", G_CALLBACK(on_pad_added), mybin);

    /* Set playbin's audio sink to be our sink bin */
    g_object_set(GST_OBJECT(pipeline), "audio-sink", mybin, NULL);
...
static void on_pad_added(GstElement *element, GstPad *pad, gpointer data) {
    ...
    if (g_str_has_prefix(padname, "video")) {
        GstPad *sinkpad = gst_element_get_static_pad(GST_ELEMENT(bin), "sink");
        if (!sinkpad)
        ...
        GstPad *ghostpad = gst_ghost_pad_new("ghostpad", pad);
        gst_pad_set_active(ghostpad, TRUE);
        gst_element_add_pad(GST_ELEMENT(bin), ghostpad);
        GstPad *sinkpad = gst_element_get_static_pad(GST_ELEMENT(bin), "sink");
        if (gst_pad_link(ghostpad, sinkpad) != GST_PAD_LINK_OK) {
