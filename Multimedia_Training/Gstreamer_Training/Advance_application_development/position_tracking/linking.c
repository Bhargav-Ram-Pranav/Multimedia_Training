...
GstElement *pipeline, *filesrc, *demuxer, *video_queue, *video_parse, *video_decode, *video_sink, *audio_queue, *audio_parse, *audio_decode, *audio_sink, *filesrc1, *demuxer1, *video_queue1, *video_decoder1, *video_sink1, *muxer;
...
int main(int argc, char *argv[]) {
    ...
    if (!pipeline || !filesrc || !demuxer || !video_queue || !video_parse || !video_decode || !video_sink || !audio_queue || !audio_parse || !audio_decode || !audio_sink || !filesrc1 || !demuxer1 || !video_queue1 || !video_decoder1 || !muxer) {
    ...
    g_object_set(video_sink, "location", "/home/bhargav/Documents/gstreamer_sample/advance_application_development/out.mp4", NULL);

    // Add all elements to the pipeline
    gst_bin_add_many(GST_BIN(pipeline), filesrc, demuxer, video_queue, video_parse, video_decode, video_sink, audio_queue, audio_parse, audio_decode, audio_sink, filesrc1, demuxer1, video_queue1, video_decoder1, muxer, NULL);
    ...
    /*if (!gst_element_link_many(video_queue1, video_decoder1, NULL)) {
        g_printerr("Video elements could not be linked.\n");
        gst_object_unref(pipeline);
        return -1;
    }*/
    ...
    /*if (!gst_element_link_many(audio_queue, audio_parse, audio_decode, NULL)) {
    ...
    // Create dynamic pads for the muxer
