...
/*static GstPadProbeReturn frame_probe(GstPad *pad, GstPadProbeInfo *info, gpointer user_data) {
    ...
    if (gst_video_info_from_caps(&video_info, gst_pad_get_current_caps(pad)) &&
            gst_video_frame_map(&vframe, &video_info, buffer, GST_MAP_READ)) {
        ...
        gboolean square_detected = FALSE; // Flag to track if a square is detected
        ...
        for (h = 0; h < height; ++h) {
            for (w = 0; w < width; ++w) {
                ...
                if (w >= square_top_left_x && w <= square_bottom_right_x &&
                    ...
                    y_pixel[0] = 255; // Y component (max value)
        ...
        if (square_detected) {
            // Save the frame as an image using filesink
            GstBuffer *copy_buffer = gst_buffer_copy(buffer);
	    GstSample *sample = gst_sample_new(copy_buffer, NULL, NULL, NULL);

	    // Assuming "sink" is the pad of the filesink element
	    GstPad *sink_pad = gst_element_get_static_pad(sink, "sink");

	    gst_pad_push(sink_pad, sample);
	...
	if (square_detected) {
		...
		// Extract the GstBuffer from the GstSample
		GstBuffer *sample_buffer = gst_sample_get_buffer(sample);

		// Assuming "sink" is the pad of the filesink element
		GstPad *sink_pad = gst_element_get_static_pad(sink, "sink");

		gst_pad_push(sink_pad, sample_buffer);

		// Unreference the GstPad and GstSample
		gst_object_unref(sink_pad);
        ...
        for (h = 0; h < height; ++h) {
            for (w = 0; w < width; ++w) {
                ...
                if ((w >= square_top_left_x && w <= square_bottom_right_x &&
                    ...
                    // Set Y, U, and V values to black for non-border pixels
    ...
    if (gst_video_info_from_caps(&video_info, gst_pad_get_current_caps(pad)) &&
        ...
        for (h = 0; h < height; ++h) {
            for (w = 0; w < width; ++w) {
                ...
                if (w >= square_top_left_x && w <= square_bottom_right_x &&
                    h >= square_top_left_y && h <= square_bottom_right_y) {
                    // Keep the pixel values inside the square
                } else {
                    // Set Y, U, and V values to black for pixels outside the square
                    y_pixel[0] = 0; // Y component
                    uv_pixel[0] = 128; // U component (or the first byte of UV)
                    uv_pixel[1] = 128; // V component (or the second byte of UV)
                }
