...
static GstPadProbeReturn frame_probe(GstPad *pad, GstPadProbeInfo *info, gpointer user_data) {
	...
	if (gst_video_info_from_caps(&video_info, gst_pad_get_current_caps(pad)) &&
		...
		g_print("No of planes are:%u\n",planes);
		guint16 *yuv_pixels = GST_VIDEO_FRAME_PLANE_DATA(&vframe, 0); // Y plane

		guint yuv_stride = GST_VIDEO_FRAME_PLANE_STRIDE(&vframe, 0); // Y plane stride
		guint pixel_stride = 3; // In RGB15, YUV values are interleaved every 3 bytes
		...
		for (h = 0; h < height; ++h) {
			for (w = 0; w < width; ++w) {
				guint16 *rgb_pixel = yuv_pixels + (h * width + w);
				...
				if ((w == square_top_left_x || w == square_bottom_right_x) &&
					...
					rgb_pixel[0] = 0; // Y component
					rgb_pixel[1] = 128; // Y component
					rgb_pixel[2] = 128; // Y component
...
int main(int argc, char *argv[]) 
{
	...
	g_object_set(G_OBJECT(sink), "location", "output.rgb", NULL);
